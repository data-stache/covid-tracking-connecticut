...
p_policy <- policy %>%
  ...
  geom_hline(yintercept=0, col = "grey40", size = .2) +
  geom_line(aes(y = stringency_index), size = .2, col="orange4") +
  ggtitle("Connecticut Covid Stringency Index",
          subtitle = "Connecticut Coivd Restriction Stringency") +
...
grid.arrange(p_new_case, p_policy, ncol = 1)

p_width <- 9
p_height <- (9/16) * p_width  

P <- arrangeGrob(p_new_case, p_policy, ncol = 1)
ggsave(paste("figs/cases-against-policy", tdy_date, ".png", sep = ""),
       P,
       width = p_width,
       height = p_height,
       dpi = "retina")
